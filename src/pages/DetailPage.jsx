...
const DetailPage = () => {
  ...
  // Función para regresar utilizando los filtros guardados, incluyendo "especializacion" si existe
  const handleBack = useCallback(() => {
    if (filters && filters.provincia && filters.canton && filters.area) {
      let url = `/resultados?provincia=${filters.provincia}&canton=${filters.canton}&area=${filters.area}`;
      if (filters.especializacion) {
        url += `&especializacion=${filters.especializacion}`;
      }
      navigate(url);
    } else {
      navigate(-1);
  ...
  useEffect(() => {
    // Se agrega una entrada extra en el historial para interceptar el retroceso
  ...
  // Función para manejar el clic en Correo
  const handleEmailClick = () => {
    if (user.email) {
      window.open(`mailto:${user.email}`, '_blank');
    } else {
      alert("No se encuentra disponible el correo electrónico para este especialista.");
    }
  };

  // Función para manejar el clic en Llamar
  ...
  const ubicacion = [user.provincia, user.canton, user.distrito]
  ...
  return (
    <div className="bg-gradient-to-b from-slate-50 to-slate-100 min-h-screen">
      <div className="container mx-auto p-4">
          ...
          <div className="bg-gradient-to-r from-blue-700 to-blue-500 text-white p-6">
            ...
            <div className="flex items-center mt-2">
              ...
              <p className="font-medium">
                {Array.isArray(user.areaTrabajo)
                  ? user.areaTrabajo.join(", ")
                  : user.areaTrabajo}
              </p>
            </div>
          </div>

          <div className="lg:flex flex-wrap p-6">
            ...
            <div className="flex-1 lg:pr-8">
              ...
              <div className="bg-slate-50 rounded-lg p-4 shadow-sm mt-6">
                ...
                {user.certificados?.length > 0 ? (
                  ...
                  <p className="text-gray-500 italic text-center">No se encuentran certificaciones disponibles.</p>
                )}
              </div>
            </div>
        ...
        {/* Sección de calificaciones - Comentada pero mejorada por si usted desea activarla */}
        {/*
          <section className="bg-slate-50 p-6 m-6 rounded-xl shadow-sm">
            ...
            {promedio !== null ? (
              ...
              <p className="mt-3 text-gray-500 italic">No se encuentran calificaciones disponibles.</p>
            )}
          </section>
        */}
        {/* Disclaimer */}
